...
void
bstack(Monitor *m) {
	int w, h, mh, mx, tx, ty, tw, framecount, rw, rh, ig, oh, ov, rx, ry;
	unsigned int i, n;
	Client *c;
...
bstackhoriz(Monitor *m) {
	int w, mh, mx, tx, ty, th, framecount, rw, rh, ig, oh, ov, rx, ry;
	unsigned int i, n;
	Client *c;
...
void
grid(Monitor *m) {
	int rows, framecount, rw, rh, ig, oh, ov, rx, ry;
...
tile(Monitor *m)
{
	...
	if (n > m->nmaster)
		mw = m->nmaster ? rw * m->mfact - ig / 2 : 0;
	else {
		mw = rw;
		if (n > 1 && n < m->nmaster) {
			m->nmaster = n;
	...
	for (i = my = ty = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			...
			h = (rh - my + ig) / (MIN(n, m->nmaster) - i) - ig;
			...
			h = (rh - ty + ig) / (n - i) - ig;
            animateclient(c, rx + mw + ig * (m->nmaster != 0), ry + ty, rw - mw - (2*c->bw) - ig * (m->nmaster != 0), h - (2*c->bw), framecount, 0);
			if (ty + HEIGHT(c) < rh)
				ty += HEIGHT(c) + ig;
		}
}
