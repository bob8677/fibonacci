...
void bstack(Monitor *m) {
  ...
  for (i = mx = 0, tx = m->wx, c = nexttiled(m->clients); c;
    ...
    if (i < m->nmaster) {
      ...
      animateclient(c, m->wx + mx, m->wy, w - (2 * c->bw), mh - (2 * c->bw),
                    framecount, 0);
      mx += WIDTH(c);
    } else {
      h = m->wh - mh;
      animateclient(c, tx, ty, tw - (2 * c->bw), h - (2 * c->bw), framecount,
                    0);
      if (tw != m->ww)
        tx += WIDTH(c);
    }
...
void monocle(Monitor *m) {
  ...
  for (c = nexttiled(m->clients); c; c = nexttiled(c->next)) {
    if (animated && c == selmon->sel) {
      animateclient(c, m->wx, m->wy, m->ww - 2 * c->bw, m->wh - 2 * c->bw, 7,
                    0);
      continue;
    }
  ...
  for (i = my = ty = 0, c = nexttiled(m->clients); c;
    ...
    if (i < m->nmaster) {
      ...
      if (n == 2) {
        tmpanim = animated;
        animated = 0;
        animateclient(c, m->wx, m->wy + my, mw - (2 * c->bw), h - (2 * c->bw),
                      framecount, 0);
        animated = tmpanim;
      } else {
        animateclient(c, m->wx, m->wy + my, mw - (2 * c->bw), h - (2 * c->bw),
