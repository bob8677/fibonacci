...
void directionfocus(const Arg *arg) {
  ...
  for (c = m->clients; c; c = c->next) {
    ...
    if (c == sc || (direction == 0 && cy > sy) || (direction == 1 && cx < sx) ||
        (direction == 2 && cy < sy) || (direction == 3 && cx > sx))
      continue;
...
void applysnap(Client *c, Monitor *m) {
  ...
  case 3:
    ...
    break;
  case 4:
    checkanimate(c, m->mx + m->mw / 2, mony + m->mh / 2, m->mw / 2, m->wh / 2,
                 7, 0);
    break;
  case 5:
...
void showoverlay() {
  ...
  if (c->islocked) {
    ...
    case 0:
      resize(c, selmon->mx + 20, selmon->my + yoffset - c->h, selmon->ww - 40,
             c->h, True);
      break;
    case 1:
      ...
      break;
    case 2:
      resize(c, selmon->mx + 20, selmon->my + selmon->mh, selmon->ww - 40, c->h,
             True);
      break;
    case 3:
  ...
  if (c->islocked) {
    ...
    case 0:
      ...
      break;
    case 1:
      animateclient(c, selmon->mx + selmon->mw - c->w, selmon->my + 40, 0, 0,
                    15, 0);
      break;
    case 2:
      animateclient(c, selmon->mx + 20, selmon->my + selmon->mh - c->h, 0, 0,
                    15, 0);
      break;
    case 3:
...
void hideoverlay() {
  ...
  if (c->islocked) {
    ...
    case 0:
      ...
      break;
    case 1:
      animateclient(c, selmon->mx + selmon->mw, selmon->mx + selmon->mw, 0, 0,
                    15, 0);
      break;
    case 2:
...
void arrangemon(Monitor *m) {
  ...
  if (m->fullscreen) {
    ...
    if (m->fullscreen->isfloating)
      savefloating(selmon->fullscreen);
    resize(m->fullscreen, m->mx, m->my + (m->showbar * bh), m->mw - (tbw * 2),
           m->mh - (m->showbar * bh) - (tbw * 2), 0);
  }
}
...
void drawbar(Monitor *m) {
  ...
  for (i = 0; i < LENGTH(tags); i++) {
    ...
    if (i == selmon->gesture - 1) {
      ...
      drw_text(drw, x, 0, w, bh, lrpad / 2, (showalttag ? tagsalt[i] : tags[i]),
               urg & 1 << i, roundw);

    } else {
      drw_text(drw, x, 0, w, bh, lrpad / 2, (showalttag ? tagsalt[i] : tags[i]),
               urg & 1 << i, drw->scheme == scheme[SchemeNorm] ? 0 : 4);
    }
    x += w;
  }
  w = blw = 60;
  ...
  if ((w = m->ww - sw - x - stw) > bh) {
    if (n > 0) {
        ...
        if (m->sel == c) {
          ...
          if (TEXTW(c->name) < (1.0 / (double)n) * w - 64) {
            drw_text(drw, x, 0, (1.0 / (double)n) * w, bh,
                     ((1.0 / (double)n) * w - TEXTW(c->name)) * 0.5, c->name, 0,
                     4);
          } else {
            drw_text(drw, x, 0, (1.0 / ((double)n) * w), bh, lrpad / 2 + 20,
                     c->name, 0, 4);
          }
          ...
          if (!c->islocked) {
            ...
            if (selmon->gesture != 12) {
              ...
              XFillRectangle(drw->dpy, drw->drawable, drw->gc, x + 6,
                             (bh - 20) / 2 - 2, 20, 16);
            ...
            drw_setscheme(drw, scheme[SchemeAddActive]);
            XSetForeground(drw->dpy, drw->gc, drw->scheme[ColBg].pixel);
            XFillRectangle(drw->dpy, drw->drawable, drw->gc, x + bh / 6, 4, 20,
                           16);
            XSetForeground(drw->dpy, drw->gc, drw->scheme[ColFloat].pixel);
            XFillRectangle(drw->dpy, drw->drawable, drw->gc, x + bh / 6, 20, 20,
                           4);
          }
          ...
          if (TEXTW(c->name) < (1.0 / (double)n) * w) {
            ...
            drw_text(drw, x, 0, (1.0 / (double)n) * w, bh, lrpad / 2, c->name,
                     0, 0);
          }
          x += (1.0 / (double)n) * w;
        }
      ...
      if (!selmon->clients) {
        int titlewidth = TEXTW("Press space to launch an application") < m->btw
                             ? TEXTW("Press space to launch an application")
                             : (m->btw - bh);
        drw_text(drw, x + bh + ((m->btw - bh) - titlewidth + 1) / 2, 0,
                 titlewidth, bh, 0, "Press space to launch an application", 0,
                 0);
      }
    }
  }
    ...
    if (!selmon->sel) {
      for (i = 0; i < LENGTH(dkeys); i++) {
        if ((code = XKeysymToKeycode(dpy, dkeys[i].keysym)))
          for (j = 0; j < LENGTH(modifiers); j++)
            XGrabKey(dpy, code, dkeys[i].mod | modifiers[j], root, True,
                     GrabModeAsync, GrabModeAsync);
      }
    }
  }
  ...
  if (!selmon->sel) {
    for (i = 0; i < LENGTH(dkeys); i++) {
      if (keysym == dkeys[i].keysym &&
          CLEANMASK(dkeys[i].mod) == CLEANMASK(ev->state) && dkeys[i].func)
    ...
    case MotionNotify:
      ...
      if (!c->isfloating && selmon->lt[selmon->sellt]->arrange &&
        ...
        if (ev.xmotion.x_root > c->x && ev.xmotion.x_root < c->x + c->w &&
            ev.xmotion.y_root > c->y && ev.xmotion.y_root < c->y + c->h) {
      ...
      if (ev.xmotion.x_root < selmon->mx ||
        ...
        if ((m = recttomon(ev.xmotion.x_root, ev.xmotion.y_root, 2, 2)) !=
            selmon) {
          pausedraw = 1;
          XRaiseWindow(dpy, c->win);
          sendmon(c, m);
...
int resizeborder(const Arg *arg) {
  ...
  do {
    XMaskEvent(
        dpy, MOUSEMASK | ExposureMask | KeyPressMask | SubstructureRedirectMask,
        &ev);
    switch (ev.type) {
    case ConfigureRequest:
    case Expose:
...
void dragmouse(const Arg *arg) {
  ...
  if (tempc->isfullscreen &&
      !tempc->isfakefullscreen) /* no support moving fullscreen windows by mouse
                                 */
    return;
  if (!getrootptr(&x, &y))
    return;
  ...
  do {
    ...
    case MotionNotify:
      ...
      if (!sinit) {
        ...
        if ((abs((starty - ev.xmotion.y_root) * (starty - ev.xmotion.y_root)) +
             abs((startx - ev.xmotion.x_root) * (startx - ev.xmotion.x_root))) >
            4069) {
          dragging = 1;
...
void resetoverlaysize() {
  ...
  case 1:
    ...
    break;
  case 2:
    resize(c, selmon->mx + 20, selmon->my + selmon->mh - c->h, selmon->ww - 40,
           (selmon->wh) / 3, True);
    break;
  case 3:
...
void dragrightmouse(const Arg *arg) {
  ...
  if (dragging) {
    ...
    if (tempc == selmon->overlay) {
      switch (selmon->overlaymode) {
      case 0:
        XWarpPointer(dpy, None, root, 0, 0, 0, 0, tempc->x + (tempc->w / 2),
                     tempc->y + tempc->h);
        break;
      case 1:
  ...
  do {
    ...
    case MotionNotify:
      ...
      if (corner != 7 && corner != 3) {
        ...
        nh = MAX(
            vertcorner ? (ocy2 - ny) : (ev.xmotion.y - ocy - 2 * c->bw + 1), 1);
      } else {
        ny = c->y;
        nh = c->h;
      }
...
void commandprefix(const Arg *arg) { tagprefix = arg->ui; }
...
void togglefakefullscreen(const Arg *arg) {
  if (selmon->sel->isfullscreen) {
    if (selmon->sel->isfakefullscreen) {
      resizeclient(selmon->sel, selmon->mx + borderpx, selmon->my + borderpx,
                   selmon->mw - 2 * borderpx, selmon->mh - 2 * borderpx);
  ...
  if (animated) {
    ...
    for (tempc = selmon->clients; tempc; tempc = tempc->next) {
      if (tempc->tags & 1 << (selmon->pertag->curtag - 2) &&
          ...
          selmon->pertag->ltidxs[selmon->pertag->curtag - 1][0]->arrange !=
              NULL) {
        if (!tmpcounter) {
          tmpcounter = 1;
          tempc->x = tempc->x - 200;
...
void animright(const Arg *arg) {
  ...
  if (animated) {
    for (tempc = selmon->clients; tempc; tempc = tempc->next) {
      if (tempc->tags & 1 << selmon->pertag->curtag && !tempc->isfloating &&
          selmon->pertag &&
          selmon->pertag->ltidxs[selmon->pertag->curtag + 1][0]->arrange !=
              NULL) {
        if (!tmpcounter) {
